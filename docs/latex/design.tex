...
del sistema software EasyLibrary. Il documento costituisce una guida per l’implementazione del software ed è destinato a supportare gli sviluppatori durante l’intero processo di sviluppo.
...
I diagrammi di sequenza sono utilizzati per dimostrare il funzionamento dinamico del sistema e per descriverne con precisione i meccanismi di implementazione agli sviluppatori.
...
singoli componenti software. La radice dell'albero è la classe \emph{Biblioteca}, composta dai
diversi archivi di dati correlati.
Al fine di poter salvare le informazioni della \emph{Biblioteca} nel file di appoggio specificato,
tutto l'albero delle classi deve implementare l'interfaccia standard \emph{java.io.Serializable}.
Questo requisito non è stato specificato nei diagrammi delle classi per motivi di leggibilità.
...
\paragraph{} Le funzionalità comuni ai vari gestori sono state consolidate in una singola classe astratta, \emph{GestoreOrdinato}, al fine di applicare il principio \emph{DRY} (\emph{Don’t Repeat Yourself}). La classe \emph{GestoreOrdinato} implementa le funzionalità di registrazione, rimozione e modifica per elementi ordinati in una lista secondo il loro ordine naturale. Le tre specializzazioni ereditano il comportamento comune e definiscono esclusivamente le parti specifiche, contribuendo a ridurre la complessità del codice e a mantenere la progettazione semplice, in accordo con il principio \emph{KISS} (\emph{Keep It Simple}).
...
\subsubsection{Scelte progettuali}
\paragraph{} Le scelte di progettazione compiute nell'ambito dati sono state pensate al fine di aumentare il livello di coesione e diminuire il livello di accoppiamento tra le diverse strutture dati. Ogni classe raggruppa attributi e metodi strettamente correlati al proprio scopo, aderendo al principio di \emph{separazione delle preoccupazioni}. Inoltre, per le specifiche di \emph{Biblioteca} e \emph{GestoreOrdinato}, tutte le classi esposte in questa sezione implementano le interfaccie standard \emph{java.io.Serializable} e \emph{java.util.Comparable} (eccetto).
...
\umlbox{
	\includesvg[width=1.1\textwidth, inkscapelatex=false]{../uml/DiagrammiSequenza/UMLinserisciUtente}
}

\newpage
\subsection{UC-11 -- Restituzione di un prestito}
\umlbox{
	\includesvg[width=1.1\textwidth, inkscapelatex=false]{../uml/DiagrammiSequenza/UMLRimuoviPrestito}
}

\newpage
\subsection{UC-12 -- Ricerca di libri}
\umlbox{
	\includesvg[width=1.1\textwidth, inkscapelatex=false]{../uml/DiagrammiSequenza/UMLRicercaLibro}
}

\newpage
\subsection{UC-14 -- Modifica di un libro}
\umlbox{
	\includesvg[width=1.1\textwidth, inkscapelatex=false]{../uml/DiagrammiSequenza/UMLModificaLibro}
}

\newpage
\subsection{UC-15 -- Rimozione di un libro}
\umlbox{
	\includesvg[width=1.1\textwidth, inkscapelatex=false]{../uml/DiagrammiSequenza/UMLRimuoviLibro}
}
...
\umlbox{
	\includesvg[width=1\textwidth, inkscapelatex=false]{../uml/DiagrammiAttività/DiagrammaModificaUtente}
}

\newpage
\subsection{UC-8 -- Rimozione di un utente}
\umlbox{
	\includesvg[width=1\textwidth, inkscapelatex=false]{../uml/DiagrammiAttività/DiagrammaEliminaUtente}
}

\newpage
\subsection{UC-9 -- Ricerca di utenti}
\umlbox{
	\includesvg[width=0.8\textwidth, inkscapelatex=false]{../uml/DiagrammiAttività/DiagrammaRicercaUtente}
}

\newpage
\subsection{UC-10 -- Registrazione di un prestito}
\umlbox{
	\includesvg[width=1\textwidth, inkscapelatex=false]{../uml/DiagrammiAttività/DiagrammaAttivitàPrestito}
}

\newpage
\subsection{UC-11 -- Restituzione di un prestito}
\umlbox{
	\includesvg[width=0.8\textwidth, inkscapelatex=false]{../uml/DiagrammiAttività/DiagrammaAttivitàRestituzione}
}
