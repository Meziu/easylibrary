...
Come si evince dal diagramma delle classi esposto, la suddivisione delle responsabilità funzionali
dell'applicativo forma una struttura simile a quella di un albero,
pensata per abbattere le interdipendenze tra le varie sezioni del sistema (in modo da garantire l'\emph{ortogonalità}) secondo il principio della
\emph{separazione delle preoccupazioni}, in modo da migliorare la coesione dei
...
Dai requisiti analizzati si è potuta riconoscere un insieme di funzionalità
comuni alle 3 principali collezioni di dati (archivio di utenti, libri e prestiti). Le funzionalità comuni
sono state estratte in un interfaccia, \emph{Archiviabile}, che permette la registrazione, rimozione e modifica di
dati generici. Per gli archivi con indice univoco (utenti e libri), è inoltre disponibile un ulteriore interfaccia \emph{Indicizzabile}, estensione di \emph{Archiviabile}, che permette l'ottenimento diretto di un valore tramite chiave. In tal modo, si è potuto far uso del principio di \emph{inversione delle dipendenze},
trattando la \emph{Biblioteca} non come una composizione di classi concrete, ma di riferimenti ad interfacce
generiche ed implementabili da classi che funzionano in modo diverso e separato.
Si è deciso di consolidare le funzionalità comuni a i diversi Gestori in una singola implementazione, data
dalla classe astratta \emph{Gestore}, in modo da applicare il principio \emph{DRY} (\emph{Don’t Repeat Yourself}), che
viene poi estesa con funzionalità e comportamenti specifici dalle 3 diverse specializzazioni. Ciò ha portato
anche ad un grande abbattimento delle implementazioni specifiche per classe \emph{Gestore}, semplificando
le diverse implementazioni specifiche come per principio \emph{KISS} (\emph{Keep It Simple, Stupid}).
Con uso rigido di incapsulamento, è stato possibile abbattere fortemente l'accoppiamento tra le classi.
