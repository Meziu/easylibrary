...
\requisitebox{IF-3.2}{Modifica di un prestito}{Il sistema deve permettere al gestore di modificare la data di scadenza di un prestito già registrato.
	Per effettuare la modifica, il gestore ricerca il prestito desiderato all’interno del sistema e aggiorna il campo relativo alla data di restituzione. La nuova data non può essere precedente alla data corrente. In caso di inserimento di una data non valida il sistema deve lanciare un messaggio di errore impedendo la modifica.}{Nuova data di scadenza del prestito.}{Nessuno.}{Il prestito deve essere attivo.}{La data di scadenza del prestito deve essere modificata e salvata. Le modifiche sono riflesse nell’archivio e rese disponibili per il salvataggio (DF-2.1).}{bassa}
...
\begin{enumerate}[label=4A.\arabic*]
	...
	\item Il sistema mostra un messaggcio di errore.
