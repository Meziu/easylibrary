...
public class GenericController implements Initializable {
    ...
    public <T, S> TableColumn<T, S> createNewColumn(TableView<T> tableView, String columnTitle, String propertyName) throws RuntimeException {
        ...
        if (columnTitle == null || propertyName == null) {
            throw new IllegalArgumentException("Titolo o nome propriet√† non possono essere null.");
        }

        // 2. Creazione della nuova colonna generica
        TableColumn<T, S> newColumn = new TableColumn<>(columnTitle);

        // 3. Associazione della colonna alla propriet√† del modello
        // Se PropertyValueFactory non riesce a trovare la propriet√†, lancia una RuntimeException
        try {
            newColumn.setCellValueFactory(new PropertyValueFactory<>(propertyName));
        } catch (Exception e) {
            // Lanciamo una RuntimeException pi√π specifica per l'errore di configurazione
            throw new RuntimeException("Errore di configurazione della colonna: impossibile trovare la propriet√† '"
    ...
    private void add(ActionEvent event) {
        if (pageType == null) {
            System.err.println("Errore: Tipo di pagina (pageType) non √® stato impostato. Impossibile aprire il popup.");
            return;
        }
    ...
    private void remove(ActionEvent event) {
        ...
        if (pageType.equals("A")) { // "A" Ë la pagina Utenti
            try {
                ...
                // NOTA: Il filtro non ha bisogno di essere rieseguito perchÈ l'elemento
                // rimosso non era pi˘ nella lista completa da filtrare. La rimozione diretta
                // dalla UTENTI_MODEL Ë sufficiente.
                new Alert(AlertType.INFORMATION, "Utente rimosso con successo.").show();

            } catch (ClassCastException e) {
                System.err.println("Errore di casting: L'elemento selezionato non Ë un Utente.");
            } catch (Exception e) {
