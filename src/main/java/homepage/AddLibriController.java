    ...
    private void handleConferma(ActionEvent event) {
        ...
        try {
            ...
            try {
                ...
                new Alert(Alert.AlertType.ERROR, "L'anno di pubblicazione non � un formato valido (es. 2023).").show();
                return;
            }
            
            // Verifica che l'anno non sia nel futuro
            if (annoPubblicazione.isAfter(Year.now())) {
                new Alert(Alert.AlertType.ERROR, "L'anno di pubblicazione non pu� essere nel futuro.").show();
                return;
            }
