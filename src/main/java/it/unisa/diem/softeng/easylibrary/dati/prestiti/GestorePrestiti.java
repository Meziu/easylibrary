...
public class GestorePrestiti extends Gestore<Prestito> {
     ...
     * prendendo per riferimento gli archivi da utilizzare come appoggio per i
     * prestiti da registrare.
     *
     * @param\[in] archivioUtenti Riferimento ad un archivio indicizzabile di
     * utenti.
     * @param\[in] archivioLibri Riferimento ad un archivio indicizzabile di
     * libri.
     */
     ...
     *
     * Aggiunge un nuovo prestito all'archivio, controllando che i suoi dati
     * siano coerenti con l'archivio di utenti e libri specificati nel
     * costruttore.
     *
     * @param\[in] p Il prestito da registrare.
     *
     * @throws ValoreNonPresenteException Se ISBN o Matricola specificati nel
     * prestito non sono presenti negli archivi di utenti e libri.
     *
     * @throws NessunaCopiaDisponibileException se il libro non ha copie
     * disponibili
     *
     ...
     *
     * Cerca il prestito nella lista e ne aggiorna lo stato in "RESTITUITO". Il
     * prestito viene rimosso dalla lista di prestiti dell'Utente e la copia
     ...
     * @post Se presente, il prestito viene rimosso dalla lista dei prestiti
     * attivi dell'utente con la Matricola corrispondente nell'archivio di
     * supporto.
     * @post Se presente, il numero di copie disponibili del libro con l'ISBN
     * corrispondente nell'archivio di supporto viene incrementato di 1.
     */
    @Override
    public void rimuovi(Prestito p) {
        ...
        // Restore strutture di utente e libro per ritornare
        // allo stato prima del prestito, se presenti negli archivi.
        Utente utente = archivioUtenti.ottieni(p1.getMatricola());
        Libro libro = archivioLibri.ottieni(p1.getISBN());

        if (utente != null) {
            archivioUtenti.modifica(utente, u -> {
                u.rimuoviPrestito(p1);
            });
        }

        if (libro != null) {
            archivioLibri.modifica(libro, l -> {
                l.setCopieDisponibili(l.getCopieDisponibili() + 1);
            });
        }
    }
